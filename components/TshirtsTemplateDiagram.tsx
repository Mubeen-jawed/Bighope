...
const TSHIRT: Record<Cat, { label: string; sizes: { size: string; w: string; h: string }[] }> = {
  youth: {
    label: 'Youth',
    sizes: [
...
const SHORTS: Record<Cat, { label: string; sizes: { size: string; waist: string; out: string }[] }> = {
  youth: {
    label: 'Youth',
    sizes: [
...
function AHead({ x, y, d }: { x: number; y: number; d: 'L' | 'R' | 'U' | 'D' }) {
  const pts =
    d === 'L' ? `${x},${y} ${x + AH},${y - 4} ${x + AH},${y + 4}` :
    d === 'R' ? `${x},${y} ${x - AH},${y - 4} ${x - AH},${y + 4}` :
    d === 'U' ? `${x},${y} ${x - 4},${y + AH} ${x + 4},${y + AH}` :
...
function Badge({ cx, cy, label }: { cx: number; cy: number; label: string }) {
  ...
  return (
    <g>
      <rect x={cx - bw / 2} y={cy - 12} width={bw} height={24} rx={12} fill={NAVY} />
      {twoLine ? (
        <>
          <text x={cx} y={cy - 3} textAnchor="middle" dominantBaseline="middle"
            fill="white" fontSize="7" fontWeight="bold">{lines[0]}</text>
          <text x={cx} y={cy + 7} textAnchor="middle" dominantBaseline="middle"
            fill="white" fontSize="7" fontWeight="bold">{lines[1]}</text>
        </>
      ) : (
        <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle"
          fill="white" fontSize="7.5" fontWeight="bold">{label}</text>
      )}
    </g>
  );
}
...
function Val({ cx, cy, v }: { cx: number; cy: number; v: string }) {
  ...
  return (
    <g>
      <rect x={cx - w / 2} y={cy - 11} width={w} height={22} rx={5} fill={RED} />
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle"
        fill="white" fontSize="11" fontWeight="bold">{text}</text>
    </g>
  );
}
...
function TshirtDiagram({ width, height }: { width: string; height: string }) {
  return (
    <svg viewBox="0 0 320 308" className="w-full" aria-label="T-shirt measurement diagram">
      ...
      <path
        ...
        fill="#e5e7eb" stroke="#9ca3af" strokeWidth="1.5" strokeLinejoin="round"
      />
      {/* Dark collar trim */}
      <path
      ...
      <line x1="240" y1="144" x2="240" y2="156" stroke={RED} strokeWidth="1.5" />
      {/* Main arrow line */}
...
function ShortsDiagram({ waist, outseam }: { waist: string; outseam: string }) {
  ...
  return (
    <svg viewBox="0 0 320 284" className="w-full" aria-label="Shorts measurement diagram">

      {/* ── Shorts body ── */}
      <path d={bodyPath} fill="#e5e7eb" stroke="#9ca3af" strokeWidth="1.5" strokeLinejoin="round" />

      {/* ── Waistband (dark) ── */}
      <rect x="78" y="50" width="164" height="24" rx="3" fill="#374151" />

      {/* Center seam hint */}
      <line x1="160" y1="74" x2="160" y2="139" stroke="#9ca3af" strokeWidth="1" strokeDasharray="4,3" />
...
export default function TshirtsTemplateDiagram() {
  ...
  const handleCat = (c: Cat) => { setCat(c); setTIdx(0); setSIdx(0); };
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-gray-50 p-6 mb-10">
      ...
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Select a category &amp; size — measurements update on the template below
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              {tData.sizes.map((s, i) => (
                <option key={s.size} value={i}>{s.size}</option>
              ))}
            </select>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <div className="rounded-lg bg-orange-50 border border-orange-100 p-3 text-center">
              <div className="text-xs text-gray-400 uppercase tracking-widest mb-0.5">Width</div>
              <div className="text-2xl font-bold text-orange-500 leading-none">{tSize.w}&quot;</div>
              <div className="text-xs text-gray-400 mt-1">pit-to-pit (flat)</div>
            </div>
            <div className="rounded-lg bg-orange-50 border border-orange-100 p-3 text-center">
              <div className="text-xs text-gray-400 uppercase tracking-widest mb-0.5">Height</div>
              <div className="text-2xl font-bold text-orange-500 leading-none">{tSize.h}&quot;</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <div className="rounded-lg bg-orange-50 border border-orange-100 p-3 text-center">
              <div className="text-xs text-gray-400 uppercase tracking-widest mb-0.5">Waist</div>
              <div className="text-2xl font-bold text-orange-500 leading-none">{sSize.waist}&quot;</div>
              <div className="text-xs text-gray-400 mt-1">flat across band</div>
            </div>
            <div className="rounded-lg bg-orange-50 border border-orange-100 p-3 text-center">
              <div className="text-xs text-gray-400 uppercase tracking-widest mb-0.5">Outseam</div>
              <div className="text-2xl font-bold text-orange-500 leading-none">{sSize.out}&quot;</div>
      ...
      <p className="text-xs text-gray-400 mt-5 text-center italic">
        All measurements in inches (flat lay). Width = pit-to-pit = half chest circumference.
        Contact us for CM charts or custom sizing.
      </p>
    </div>
  );
}
